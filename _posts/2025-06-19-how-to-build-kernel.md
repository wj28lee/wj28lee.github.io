...
tags: [linux, kernel]
---
종종 리눅스 커널의 최신 기능 실험(?)을 위해, github repo.에서 코드를 다운로드 받아 빌드/설치하는 경우가 있다. 사실 방법이야 단순하지만, 커널 개발자가 아니라면 굳이 모든 history를 다운로드 받을 필요는 없다. 시간도 오래 걸리고.
...
만일 kernel command line이나 부팅해야 하는 커널 이미지 순서에 대한 변경이 필요하다면, 아래 단계를 참고하자.
```bash
